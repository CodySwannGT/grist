/**
 * Threnos the Unmade free-vs-wield site verification (UAT) suite — the Bound-site half
 * of the Validation Journey for #132 (PRD #43 FR5 / AC7). Drives the in-game
 * `window.__VERIFY__` bridge against the live build to prove EMPIRICALLY (unit /
 * typecheck / lint alone are NOT acceptable per the issue) that the Wrack sites
 * Threnos, and reaching that site lets the player face the region's free-vs-wield
 * decision — quieting the Sundering's rawest wound, or drawing raw entropy from it —
 * resolved and persisted across a genuine reload:
 *
 * - [EVIDENCE: threnos-bound-resolves] — freeing Threnos grants the WEAKER shard with
 *   karma+ and NO corruption, observed scene-agnostically via
 *   `__VERIFY__.openBoundSite("wrack")` / `chooseBound("free")` / `boundSite()`; then a
 *   save built from that settled choice round-trips through IndexedDB and is restored
 *   after a GENUINE page reload (`loadSave()` / `runState()` show `variant: "free"`,
 *   `karma: 1`).
 * - [EVIDENCE: threnos-bound-resolves] — the SAME site, choosing WIELD, grants the
 ...
 *
 * Threnos is the finale foreshadow — the most alien, entropy-touched power — so
 * wielding it genuinely accrues corruption (the fork is measurable) at the HEAVIEST
 * authored cost: strictly above Korrholt's openly-run reactor. Determinism: the site's
 ...
 * irrelevant and the default boot is used (mirrors `morrath-bound-site.spec.ts`). The
 * Phaser-free unit twin (`tests/logic/threnos-bound-site.test.ts`) proves the
 ...
 * persistence assertions read. Asserting the whole shape up front means a broken bridge
 * fails here, loudly, instead of silently no-op'ing through an optional chain.
 ...
 * (mirrors `morrath-bound-site.spec.ts`).
 */
...
/**
 * Build the save a settled Threnos Bound-site choice persists: Threnos carried in the
 * chosen mode (`party[].shard` + `shardMode`), the resolved `choice`, and the matching
 * karma / ledger tallies — the exact run state the player would save after facing
 * Threnos. The two variants flip the shardMode / choice / karma so the forks are
 * measurably distinct persisted payloads.
 *
 ...
 * so restoring `{ shard: threnos, variant: "wield" }` restores the WIELDED state WITH
 * its corruption (a positive, derivable rate), while `"free"` restores zero.
 * @param variant - The resolved choice ("free" or "wield").
 * @returns A complete v3 save for the settled Threnos choice.
 */
function settledSave(variant: "free" | "wield"): SaveDataV2 {
  const free = variant === "free";
  return {
    version: 3,
    party: [{ id: "wren", level: 8, shard: THRENOS, shardMode: variant }],
    grist: 18,
    inventory: [],
    learned: [],
    learning: [],
    choice: { resolved: true, shard: THRENOS, variant },
    moralLedger: {
      karma: free ? 1 : -1,
      freeChoices: free ? 1 : 0,
      wieldChoices: free ? 0 : 1,
    },
    rng: { seed: 0x77ac, state: 0x77ac },
    worldState: "reach",
 ...
 * accrues none; wield accrues the carried shard's wield rate read from the live content
 * table via a freshly-opened site. Proves the WIELDED carry restores WITH its
 * (positive) corruption — not merely the variant flag — without persisting a fractional
 * count the save validator would reject.
 * @param page - The Playwright page.
...
async function restoredCorruption(
  ...
  return page.evaluate(mode => {
    const api = window.__VERIFY__!;
    api.openBoundSite("wrack");
    api.chooseBound(mode);
    return api.boundSite()!.corruptionAccrued;
  }, variant);
}

test.describe("GRIST — Threnos the Unmade free-vs-wield site (UAT, #132)", () => {
  test.beforeEach(async ({ page }) => {
    await bootWithBridge(page);
    await page.evaluate(() => window.__VERIFY__!.clearSave());
  });

  test("[EVIDENCE: threnos-bound-resolves] freeing Threnos grants the weaker shard with karma+ and no corruption, restored from IndexedDB after reload", async ({
    page,
  }) => {
    const errors: string[] = [];
    ...
    // Reach the Wrack region's single Bound site through the template (pure data
    // through the content barrel — no engine wiring), then read the unsettled site.
    await page.evaluate(() => window.__VERIFY__!.openBoundSite("wrack"));
    const opened = await page.evaluate(() => window.__VERIFY__!.boundSite());
    expect(opened).not.toBeNull();
    // The site is exactly Threnos in the Wrack region, offering its variants.
    expect(opened!.shard).toBe(THRENOS);
    expect(opened!.regionId).toBe(WRACK);
    expect(opened!.settled).toBe(false);
    expect(opened!.freeCorruptionRate).toBe(0);
    // The wield path costs corruption — drawing raw entropy is not free (the fork).
    expect(opened!.wieldCorruptionRate).toBeGreaterThan(0);

    // Choose FREE — the weaker, corruption-free attunement (karma+): the wound quieted,
    // the end refused, the mercy.
    ...
    // The freed Threnos state survived the reload exactly — restored from IndexedDB.
  ...
  test("[EVIDENCE: threnos-bound-resolves] wielding Threnos grants the stronger carry with accruing corruption that, with its corruption, survives the reload", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("pageerror", error => errors.push(error.message));

    // Reach Threnos's site and choose WIELD — the stronger carry that accrues
    // corruption (karma−): drawing raw entropy from a piece of the end, the desecration.
    await page.evaluate(() => window.__VERIFY__!.openBoundSite("wrack"));
    const opened = await page.evaluate(() => window.__VERIFY__!.boundSite());
    expect(opened!.shard).toBe(THRENOS);
    expect(opened!.regionId).toBe(WRACK);
    ...
    // Corruption accrued is Threnos's wield rate — strictly positive (the cost).
  ...
  test("[EVIDENCE: threnos-bound-resolves] free vs wield diverge at Threnos's site, and its hash is deterministic across a reload", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("pageerror", error => errors.push(error.message));

    // Free fork: open Threnos's site + choose free, capture its digest.
    await page.evaluate(() => window.__VERIFY__!.openBoundSite("wrack"));
    await page.evaluate(() => window.__VERIFY__!.chooseBound("free"));
    const free = await page.evaluate(() => window.__VERIFY__!.boundSite());
    expect(free!.shard).toBe(THRENOS);
