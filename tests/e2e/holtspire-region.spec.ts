 ...
 * canvas, that Holtspire — the Anvil-city, House Caldecott's industrial city-state —
 ...
 * #137 `region-harness.spec.ts`): the harness is reusable per region, so the same
 * spec shape verifies Holtspire with no engine-code edit — the `?region=holtspire`
 * query routes the pure `src/logic/region` harness. The Phaser-free unit twin
 * (`tests/logic/holtspire-region.test.ts`) proves the both-states data + determinism
 * logic headlessly; this spec proves it on the live, rendered canvas. The Korrholt
 * Bound-site free-vs-wield is verified separately in
...
/**
 * Boot the Region scene directly at the Holtspire region with the bridge enabled. The
 * `?scene=region` query starts the Region scene; `?region=holtspire` selects Holtspire;
 * `?world=ashfall` (optional) boots straight into the Act II variant; and `?seed=` pins
 * the seeded RNG so the run is reproducible.
 * @param page - The Playwright page.
...
test.describe("GRIST — the Holtspire region verification (UAT, #130)", () => {
  test("[EVIDENCE: holtspire-plays-both-states] boots Holtspire to the 384x216 side-view with zero console errors and is deterministic across a reload", async ({
    ...
    // gives Holtspire its own set; asserted against the same key the framework harness
