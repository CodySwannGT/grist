/**
 * The Wrack region verification (UAT) suite — the live-canvas half of the Validation
 * Journey for #132. Boots the rendered Region scene directly at
 * `?scene=region&region=wrack` (and `&world=ashfall`) and drives it through the
 * in-game `window.__VERIFY__` bridge to prove, empirically against the live 384×216
 * canvas, that the Wrack — the Sundering coast: a broken tidal coast where the
 * Sundering's wound is rawest, home to an oblivion-cult — authored against the shipped
 * framework boots in BOTH world-states, renders cleanly, and digests deterministically
 * across a genuine page reload.
 *
 * Mirrors `tests/e2e/cinderfen-region.spec.ts` (itself a mirror of the framework's
 * #137 `region-harness.spec.ts`): the harness is reusable per region, so the same spec
 * shape verifies the Wrack with no engine-code edit — the `?region=wrack` query routes
 * the pure `src/logic/region` harness through `requestedRegion`. The Phaser-free unit
 * twin (`tests/logic/wrack-region.test.ts`) proves the both-states data + determinism
 * logic headlessly; this spec proves it on the live, rendered canvas. The Threnos
 * Bound-site free-vs-wield is verified separately in `tests/e2e/threnos-bound-site.spec.ts`.
 *
 * The Wrack DOES turn across the Reckoning (verdant → ashen) AND its encounter table
 * diverges (the retreating sea lays the sunken Choir-shrine bare), so its Reach→Ashfall
 * divergence is BOTH tone and encounters — either of which makes the resolved
 * determinism hash differ between the two boots below.
 *
 * Evidence marker (the required test title):
 * - [EVIDENCE: wrack-plays-both-states] — the region boots and reads in both the Reach
 *   (Act I) and Ashfall (Act II) world-states.
 */
import { expect, test, type Page } from "@playwright/test";

const SEEN_TIMEOUT = 15_000;
const FIXED_SEED = 0x77ac;
...
/**
 * Boot the Region scene directly at the Wrack region with the bridge enabled. The
 * `?scene=region` query starts the Region scene; `?region=wrack` selects the Wrack;
 * `?world=ashfall` (optional) boots straight into the Act II variant; and `?seed=`
 * pins the seeded RNG so the run is reproducible.
 * @param page - The Playwright page.
 * @param world - The `?world=` selector (omitted for the default `reach`).
 * @param seed - The fixed boot seed (defaults to {@link FIXED_SEED}).
 */
async function bootWrack(
  page: Page,
  world?: "ashfall",
  ...
  await page.goto(
    `/?scene=region&uat=1&seed=${seed}&region=wrack${worldQuery}`
  );
 ...
 * after each step. The sampled progression is the determinism evidence — the same seed
 * + same sequence must reproduce it byte-for-byte across a reload.
 * @param page - The Playwright page.
...
test.describe("GRIST — the Wrack region verification (UAT, #132)", () => {
  test("[EVIDENCE: wrack-plays-both-states] boots the Wrack to the 384x216 side-view with zero console errors and is deterministic across a reload", async ({
    page,
  }) => {
    const errors: string[] = [];
    ...
    await bootWrack(page);
    await expect(page.locator("canvas")).toBeVisible();

    // The harness booted the Wrack cleanly in its Reach (Act I) world-state.
    const run = await regionRun(page);
    expect(run).not.toBeNull();
    expect(run!.scene).toBe("Region");
    expect(run!.runtimeScene).toBe("region:wrack");
    expect(run!.regionId).toBe("wrack");
    ...
    // The per-region backdrop pass (#200) gives the Wrack its OWN distinct parallax
    // set (a drowned blue-teal palette variant of the CC0 Warped City layers); the
    // scene renders exactly this key, an asset the loader can resolve.
    expect(run!.backdrop).toBe("img-wrack/bg-far");
  ...
  test("[EVIDENCE: wrack-plays-both-states] boots the Wrack directly into its Ashfall world-state, observably different from Reach", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("console", message => {
    ...
    // Capture the Reach boot snapshot first (same seed) so the Ashfall divergence is a
    ...
    await bootWrack(page, "ashfall");
    await expect(page.locator("canvas")).toBeVisible();

    const ashfallRun = await regionRun(page);
    expect(ashfallRun).not.toBeNull();
    expect(ashfallRun!.scene).toBe("Region");
    expect(ashfallRun!.runtimeScene).toBe("region:wrack");
    expect(ashfallRun!.regionId).toBe("wrack");
    ...
    // Wrack turns verdant→ashen AND its Ashfall drowned-Choir encounter table differs
    // from the Reach scar table).
