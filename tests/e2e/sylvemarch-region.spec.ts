/**
 * The Sylvemarch region verification (UAT) suite — the live-canvas half of the
 * Validation Journey for #129. Boots the rendered Region scene directly at
 * `?scene=region&region=sylvemarch` (and `&world=ashfall`) and drives it through the
 * in-game `window.__VERIFY__` bridge to prove, empirically against the live 384×216
 * canvas, that the Sylvemarch — the surviving forest, the Green Mother's march —
 * authored against the shipped framework boots in BOTH world-states, renders cleanly,
 * and digests deterministically across a genuine page reload.
 *
 * Mirrors `tests/e2e/roots-region.spec.ts` (itself a mirror of the framework's #137
 * `region-harness.spec.ts`): the harness is reusable per region, so the same spec
 * shape verifies the Sylvemarch with no engine-code edit — the `?region=sylvemarch`
 * query routes the pure `src/logic/region` harness through {@link requestedRegion}.
 * The Phaser-free unit twin (`tests/logic/sylvemarch-region.test.ts`) proves the
 * both-states data + determinism logic headlessly; this spec proves it on the live,
 * rendered canvas. The Sylvath Bound-site free-vs-wield is verified separately in
 * `tests/e2e/sylvath-bound-site.spec.ts`.
 *
 * Evidence marker (the required test title):
 * - [EVIDENCE: sylvemarch-plays-both-states] — the region boots and reads in both the
 *   Reach (Act I) and Ashfall (Act II) world-states.
 */
import { expect, test, type Page } from "@playwright/test";

const SEEN_TIMEOUT = 15_000;
const FIXED_SEED = 0x5417;
...
/**
 * Boot the Region scene directly at the Sylvemarch region with the bridge enabled.
 * The `?scene=region` query starts the Region scene; `?region=sylvemarch` selects the
 * Sylvemarch; `?world=ashfall` (optional) boots straight into the Act II variant; and
...
async function bootSylvemarch(
  ...
  await page.goto(
    `/?scene=region&uat=1&seed=${seed}&region=sylvemarch${worldQuery}`
  );
...
test.describe("GRIST — the Sylvemarch region verification (UAT, #129)", () => {
  test("[EVIDENCE: sylvemarch-plays-both-states] boots the Sylvemarch to the 384x216 side-view with zero console errors and is deterministic across a reload", async ({
    page,
  }) => {
    const errors: string[] = [];
    ...
    await bootSylvemarch(page);
    await expect(page.locator("canvas")).toBeVisible();

    // The harness booted the Sylvemarch cleanly in its Reach (Act I) world-state.
    const run = await regionRun(page);
    expect(run).not.toBeNull();
    expect(run!.scene).toBe("Region");
    expect(run!.runtimeScene).toBe("region:sylvemarch");
    expect(run!.regionId).toBe("sylvemarch");
  ...
  test("[EVIDENCE: sylvemarch-plays-both-states] boots the Sylvemarch directly into its Ashfall world-state, observably different from Reach", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("console", message => {
    ...
    await bootSylvemarch(page, "ashfall");
    await expect(page.locator("canvas")).toBeVisible();

    const ashfallRun = await regionRun(page);
    expect(ashfallRun).not.toBeNull();
    expect(ashfallRun!.scene).toBe("Region");
    expect(ashfallRun!.runtimeScene).toBe("region:sylvemarch");
    expect(ashfallRun!.regionId).toBe("sylvemarch");
    ...
    // greying-march Ashfall encounter table differs from the Reach table).
