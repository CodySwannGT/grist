 ...
 * (#130, PRD #43 FR5 / AC7). The Holtspire region sites Korrholt (`content/regions` →
 * `boundSite: korrholt`), and this suite proves — headless, with ZERO Phaser imports
 ...
 * the franchise's core moral fork at its STARKEST:
 *
 * - **free** — the weaker Korrholt shard, karma+, and NO corruption (the city reactor
 *   shut down).
 * - **wield** — the stronger carry that accrues corruption (karma−): the atrocity
 *   industrialized, the Bound left running as the Anvil-city's reactor.
 *
 * Korrholt is the free-vs-wield choice at its starkest (a power harnessed openly as
 * infrastructure), so its wield cost is the HEAVIEST authored Bound — strictly above
 * the Marrow Bound's — while freeing it stays corruption-free. The unit twin proves
 * the rule headlessly; `tests/e2e/korrholt-bound-site.spec.ts` proves it persists
...
describe("Korrholt Bound data — Holtspire's Anvil-Heart (#130)", () => {
  it("the Holtspire region sites exactly Korrholt", () => {
    expect(HOLTSPIRE.boundSite).toBe(KORRHOLT);
  });

  it("free accrues no corruption (the safe attunement — the reactor shut down)", () => {
  ...
  it("is the choice at its starkest: Korrholt's wield cost is strictly above the Marrow Bound's", () => {
...
describe("chooseAtBoundSite(holtspire) — free (freed, AC scenario)", () => {
...
describe("chooseAtBoundSite(holtspire) — wield (left as the city reactor, AC7)", () => {
