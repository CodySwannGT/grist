/**
 * Unit coverage for **Holtspire** — the Anvil-city (#130), a rival industrial
 * city-state ruled by House Caldecott, authored as a {@link RegionDef} against the
 * shipped region-production framework (template #133, world-state #134, Bound-site
 * template #135, boot harness #137). This suite is the Phaser-free half of the
 * Validation Journey: it proves the authored DATA resolves correctly in both
 * world-states, sites Korrholt the Anvil-Heart, and digests deterministically through
 * the pure harness, mirroring the import style and conventions of
 * `tests/logic/sylvemarch-region.test.ts` (it imports ONLY from `../../src/content`
 * and `../../src/logic/...` — ZERO Phaser, FR9).
 *
 ...
 *   foundry, the frame-yards, the ripper row) in BOTH variants and passes
 *   {@link isCompleteRegion} / {@link validateRegion}.
 * - AC1: Ashfall is observably different from Reach — a different tone AND a
 *   different encounter set (the foundries going cold and silent across the Reckoning).
...
describe("the Holtspire region (#130)", () => {
  ...
  it("resolves its identity and key locations in the Reach (Act I) variant", () => {
    ...
    // The Reach is the loud, working anvil-city — the verdant read.
    ...
    expect(names.some(name => name.includes("great foundry"))).toBe(true);
  ...
  it("resolves a warped identity and the same landmarks in the Ashfall (Act II) variant", () => {
    ...
    // The foundries cold and silent, Caldecott a warlord remnant — the ashen read.
  ...
  it("makes Ashfall observably different from Reach (tone + encounter set)", () => {
    ...
    // region reads as a different place in Act II — the anvil-city gone cold.
