 ...
 *                                           atlas; `load.image`)
 *   assets/src/audio/**\/*.{ogg,wav,mp3}  → public/assets/audio/** (verbatim
 *                                           copy — CC0 temp music + stingers;
 *                                           `load.audio` per entry, no re-encode
 *                                           so the pipeline stays byte-reproducible
 *                                           without ffmpeg)
 *
 * The generated module exports:
 *   AtlasKeys  — one key per packed atlas.
 *   ImageKeys  — one key per standalone image (path-derived, stable).
 *   AudioKeys  — one load key per standalone audio clip (path-derived, stable).
 *   AudioManifest — {key,url} rows the Preloader iterates to `load.audio` each
 *                clip (the url carries the real extension, ogg or wav).
...
const SRC_AUDIO = join(ROOT, "assets", "src", "audio");
const OUT_ATLASES = join(ROOT, "public", "assets", "atlases");
const OUT_IMAGES = join(ROOT, "public", "assets", "images");
const OUT_AUDIO = join(ROOT, "public", "assets", "audio");

/** Audio source extensions the pipeline copies verbatim (no re-encode). */
...
/**
 * Recursively list every audio clip (by {@link AUDIO_EXTS}) under a directory,
 * sorted for determinism.
 * @param {string} dir - The directory to walk.
 * @returns {readonly string[]} Absolute file paths, sorted.
 */
function audioUnder(dir) {
  if (!existsSync(dir)) {
    return [];
  }
  return readdirSync(dir, { recursive: true, withFileTypes: false })
    .map(entry => join(dir, String(entry)))
    .filter(path => AUDIO_EXTS.some(ext => path.endsWith(ext)))
    .sort();
}
...
/**
 * Render the generated `AudioManifest` array literal ({key,url} rows the
 * Preloader iterates to `load.audio` each clip; the url carries the real
 * extension so ogg music and wav stingers load side by side).
 * @param {ReadonlyArray<{key: string, url: string}>} rows - The manifest rows.
 * @returns {string} TypeScript source.
 */
function renderAudioManifest(rows) {
  const body = rows
    .map(row => `  { key: "${row.key}", url: "${row.url}" },`)
    .join("\n");
  const inner = rows.length > 0 ? `\n${body}\n` : "";
  return `/** Audio clips to \`load.audio\` (key + url with its real extension). */\nexport const AudioManifest = [${inner}] as const;\n`;
}
 ...
 * @param {readonly string[]} audioRelPaths - Standalone audio paths (relative, with extension).
 * @returns {string} The full module source.
 */
function renderKeysModule(atlases, imageRelPaths, audioRelPaths) {
  const header = `/**
 ...
 */\n\n`;
  ...
  const audioBase = rel =>
    rel
      ...
      .split(sep)
      .join("/");
  const audioKeys = Object.fromEntries(
    audioRelPaths.map(rel => [
      identifierFor(audioBase(rel)),
      `audio-${audioBase(rel)}`,
    ])
  );
  const audioManifest = audioRelPaths.map(rel => ({
    key: `audio-${audioBase(rel)}`,
    url: `assets/audio/${rel.split(sep).join("/")}`,
  }));
  ...
  return (
    ...
    "\n" +
    renderAudioManifest(audioManifest) +
    "\n" +
...
async function build() {
  ...
  const audioRelPaths = audioUnder(SRC_AUDIO).map(path =>
    relative(SRC_AUDIO, path)
  );
  for (const rel of audioRelPaths) {
    outputs.set(
      join("public", "assets", "audio", rel),
      readFileSync(join(SRC_AUDIO, rel))
    );
  }
  outputs.set(
    join("src", "assets.ts"),
    renderKeysModule(atlases, imageRelPaths, audioRelPaths)
  );
  return { outputs };
}
